...
export default function SideMenu() {
  ...
  return (
    <aside
      className={`flex flex-col p-2 dark:bg-gray-950 transition-all duration-300 ease-in-out border-r border-couleur1/10 dark:border-white/10 ${toggleMenu ? "w-56" : "w-15"} `}
    >
      <div className="flex items-center gap-2 mb-10 overflow-hidden whitespace-nowrap">
        <div className="w-8 h-8  shrink-0">
          {" "}
          <img src={logo} alt="TurboStack" />{" "}
        </div>
        <span
          className={`text-xl font-bold text-couleur1 transition-all duration-300 ${toggleMenu ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4 pointer-events-none"}`}
        >
          TurboStack
        </span>
      </div>
      <button
        ...
        className="flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-all hover:bg-opacity-10 hover:bg-couleur1 w-fit hover:text-couleur3 text-couleur1 justify-start mb-1"
      >
        <div className="shrink-0">
          {toggleMenu ? (
            <SidebarClose size={20}></SidebarClose>
          ) : (
            <SidebarOpen size={20}></SidebarOpen>
          )}
        </div>
      </button>
      <nav className="sidemenu flex-1 space-y-2 overflow-y-scroll">
        ...
        <button
          ...
          className="flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all border-couleur7 text-couleur1 hover:bg-couleur1 hover:text-white justify-start fixed w-fit bottom-0 -right-8 z-10 dark:bg-couleur1 dark:text-white dark:border-none hover:right-2 ease-in-out"
        >
          <div className="shrink-0">
        ...
        {menuItems.map((item) => {
          ...
          const isActive =
            actualWindow === item.name ||
            isSubActive ||
            (item.name === "Project" &&
              ["New Project", "Project Home Page"].includes(actualWindow));
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <div
                    className={`flex-1 flex items-center justify-between transition-all duration-300 ${toggleMenu ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4 pointer-events-none"}`}
                  >
                    <span className="font-medium">
                      {item.label || item.name}
                    </span>
                    {editorExpanded || isSubActive ? (
                      <ChevronDown size={14} />
                    ) : (
          ...
          return (
            ...
            >
              <div className="shrink-0">{item.icon}</div>
              <span
                className={`font-medium transition-all duration-300 ${toggleMenu ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4 pointer-events-none"}`}
              >
                {item.label || item.name}
              </span>
            </div>
