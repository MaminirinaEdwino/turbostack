...
export default function APIlist() {
    ...
    // ASSUMPTION: projectName is stored in Redux when a project is selected/opened.
    // You might need to implement `state.app.currentProjectName` in your Redux store.
    ...
    useEffect(() => {
        const loadProject = async () => {
            if (!projectName) {
                setError("No project selected. Please go to the Project Dashboard and select a project.");
                setLoading(false);
                return;
            }
            try {
        ...
        return (
            <div className="flex h-screen w-full font-san bg-couleur3">
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-red-500 text-xl">
                    <p>{error}</p>
                    {!projectName && <button onClick={() => navigateTo("Project")} className="mt-4 px-4 py-2 bg-couleur1 text-white rounded-lg hover:bg-opacity-90">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                <div className="bg-white p-6 rounded-lg shadow-md border border-couleur1/10">
                    <div className="flex items-center gap-3 mb-3">
                        ...
                        <h2 className="text-xl font-bold text-couleur1">Endpoints Summary</h2>
                    </div>
                    <p className="text-gray-700">Total Endpoints: <span className="font-semibold">{totalEndpoints}</span></p>
                    <button
                ...
                <div className="bg-white p-6 rounded-lg shadow-md border border-couleur1/10">
                    <div className="flex items-center gap-3 mb-3">
                        ...
                        <h2 className="text-xl font-bold text-couleur1">Available Roles</h2>
                    </div>
                    <div className="flex flex-wrap gap-2">
                        {availableRoles.map(role => (
                            <span key={role} className="px-3 py-1 bg-couleur3 text-couleur1 rounded-full text-sm capitalize">
                                {role}
                            </span>
                        ))}
                    </div>
