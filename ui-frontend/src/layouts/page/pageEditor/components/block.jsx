...
export default function Block({ block, getIconForTag }) {
    return <div className="flex items-center gap-3" title={`${block.htmlId != "" ? "#"+block.htmlId : block.tag} ${block.inputType != "" ? block.inputType : ""}`}>
        ...
        <div title={block.htmlId != "" ? "#"+block.htmlId : block.tag}>
            <p className="text-[10px] font-black text-couleur1 dark:text-gray-400 uppercase tracking-tighter w-17 truncate">
                {block.htmlId != "" ? "#"+block.htmlId : block.tag} <span className="text-[8px]"> {block.inputType != "" && block.inputType}</span>
            </p>
