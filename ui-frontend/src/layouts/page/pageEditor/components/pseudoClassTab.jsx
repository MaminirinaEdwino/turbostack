...
import { GROUP_LIST, GROUP_LIST_ICON, PSEUDO_CLASS, STYLE_CONTROLS, TAG_STYLE_GROUPS } from "../defaultVar";
    ...
    return <>
        <div className="flex flex-col gap-6 animate-in fade-in duration-300 ">
            <div className="flex flex-col gap-2">
                ...
                >
                    {PSEUDO_CLASS.map(pc => <>
                        <option >{currentBlock?.tag}:{pc}</option>
                        {currentBlock != null && <option value={`[data-block-id="${currentBlock?.id}"]`} >[data-block-id="{currentBlock?.id}"]:{pc}</option>}
                    </>)}
                                            ...
                                            <div key={ctrl.prop} className={"flex flex-col gap-1 justify-between  my-1 " + (ctrl.grid && ctrl.grid)}>
                                                ...
                                                {ctrl.type === "number" ? (
                                                    ...
                                                    >
                                                        ...
                                                        {ctrl.options.map((opt, idx) => <button className={"flex text-xs gap-1 p-1 rounded transition-all duration-150 " + (currentValue == opt && "bg-couleur1 text-couleur3 ")} onClick={() => handlePageStyleChange(ctrl.prop, opt)}> {ctrl.optionIcon[idx]} {opt} </button>)}
                                                    </div>
                                                ) : ctrl.type == "preset" ? <div className="flex flex-wrap gap-1">
                                                    {ctrl.presetType == "color" && ctrl.option.map(opt => <button className={"w-10 h-10 rounded-full " + (currentValue == opt && " border-2 border-couleur2")} style={{ backgroundImage: opt }} onClick={() => handlePageStyleChange(ctrl.prop, opt)}>

                                                    </button>)}
                                                    {ctrl.presetType == "box-shadow" && ctrl.option.map(opt => <button className={"w-10 h-10 rounded-full " + (currentValue == opt && " bg-couleur2/20")} style={{ boxShadow: opt }} onClick={() => handlePageStyleChange(ctrl.prop, opt)}>
                                                    </button>)}
