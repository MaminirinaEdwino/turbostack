...
export default function EditEndpoint({
  ...
  const handleUpdate = (e) => {
    e.preventDefault();
    const updatedEndpoints = [...(project.rest_api?.endpoints || [])];
  ...
  const toggleModelSelection = (model) => {
    ...
    if (isSelected) {
      const paramsToRemove = model.champs.map((c) => c.nom);
      let newUri = endpoint.uri;
  ...
  const addManualField = () => {
    if (
      !manualField.nom ||
      endpoint.manual_fields.some((f) => f.nom === manualField.nom)
    )
      return;
    setEndpoint({
      ...endpoint,
      manual_fields: [...endpoint.manual_fields, manualField],
    });
  ...
  const removeManualField = (nom) => {
    setEndpoint((prev) => ({
      ...
      model: prev.model
        .map((m) =>
          m.nom === "Manual"
            ? { ...m, champs: m.champs.filter((f) => f.nom !== nom) }
            : m,
        )
  ...
  const generateTableFromManual = () => {
    ...
    if (
      availableModels.some(
        (m) => m.nom.toLowerCase() === tableName.toLowerCase(),
      )
    ) {
    ...
    setProject({
      ...project,
      bdd: { ...project.bdd, models: [...availableModels, newModel] },
    });
    setTableName("");
  };
  ...
  const toggleUriParam = (fieldName) => {
    ...
    if (isSelected) {
      newParams = endpoint.params.filter((p) => p !== fieldName);
      newUri = newUri
        .replace(`/:${fieldName}`, "")
        .replace(`:${fieldName}`, "");
    } else {
      newParams = [...endpoint.params, fieldName];
      if (!newUri.includes(`:${fieldName}`)) {
        newUri = newUri.endsWith("/")
          ? `${newUri}:${fieldName}`
          : `${newUri}/:${fieldName}`;
      }
    }
    setEndpoint({ ...endpoint, params: newParams, uri: newUri });
  };
        ...
        newModels = prev.model
          .map((m) => {
            ...
            return {
              ...m,
              champs: hasField
                ? m.champs.filter((f) => f.nom !== field.nom)
                : [...m.champs, field],
            };
          })
          .filter((m) => m.nom !== "Manual" || m.champs.length > 0);
    ...
    <form className="bg-white border border-couleur1 p-6 rounded-xl shadow-2xl flex flex-col w-137.5 max-h-[85vh] overflow-y-auto">
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        <div className="flex flex-col gap-1">
          ...
          <input
            className="border border-couleur1 p-2 rounded-lg outline-none focus:ring-2 ring-couleur1/20"
            type="text"
            value={endpoint.nom}
            onChange={(e) => setEndpoint({ ...endpoint, nom: e.target.value })}
      ...
      <div className="flex flex-col gap-1 mb-4">
        ...
        <input
          className="border border-couleur1 p-2 rounded-lg font-mono text-sm"
          type="text"
          value={endpoint.uri}
          onChange={(e) => setEndpoint({ ...endpoint, uri: e.target.value })}
        />
      </div>
      ...
      <div className="flex flex-col gap-1 mb-6">
        ...
        <div className="flex gap-2 mt-2">
          <input
            className="flex-1 border border-couleur1 p-2 rounded-lg text-sm"
            type="text"
            placeholder="Field name"
            value={manualField.nom}
            onChange={(e) =>
              setManualField({ ...manualField, nom: e.target.value })
            }
          />
          <select
            className="border border-couleur1 p-2 rounded-lg text-sm bg-white"
            value={manualField.type}
            onChange={(e) =>
              setManualField({ ...manualField, type: e.target.value })
            }
          >
          ...
          <button
            ...
            className="bg-couleur1 text-white p-2 rounded-lg"
          >
            <Plus size={18} />
          </button>
        </div>
        <div className="flex flex-wrap gap-2 mt-2">
          {endpoint.manual_fields.map((f, i) => (
            <div
              key={i}
              className="flex items-center gap-2 bg-couleur3 px-3 py-1 rounded-full border border-couleur1/20 text-xs"
            >
              <span className="font-medium">{f.nom}</span>
              <span className="opacity-50">({f.type})</span>
              <button
                type="button"
                onClick={() => removeManualField(f.nom)}
                ...
                <tbody>
                  {endpoint.manual_fields.map((f) => (
                    <tr key={f.nom} className="border-t border-blue-100">
                      <td className="py-2 text-blue-800 font-medium">
                        {f.nom}{" "}
                      ...
                      <td className="py-2 text-center">
                        <input
                          type="checkbox"
                          checked={endpoint.params.includes(f.nom)}
                          onChange={() => toggleUriParam(f.nom)}
                      ...
                      <td className="py-2 text-center">
                        <input
                          ...
                          checked={endpoint.model
                            .find((m) => m.nom === "Manual")
                            ?.champs.some((bc) => bc.nom === f.nom)}
                          onChange={() => toggleManualBodyField(f)}
                          className="accent-blue-600 cursor-pointer w-4 h-4"
                        />
                      </td>
                    </tr>
      ...
      <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
        <button
          ...
          className="px-5 py-2 rounded-lg border border-couleur1 text-couleur1 hover:bg-gray-50 transition-colors"
        >
          Cancel
        </button>
        <button
          onClick={handleUpdate}
          className="px-5 py-2 rounded-lg bg-couleur1 text-white font-semibold flex items-center gap-2 hover:bg-opacity-90 transition-all"
        >
