...
export default function EndpointNode({ data }) {
    ...
    return (
        <div className="border border-couleur1 rounded-lg overflow-hidden bg-white min-w-[200px] shadow-sm">
            <div className="border-b border-couleur1 p-2 bg-couleur3 flex justify-between items-center gap-4">
            ...
            <div className="p-2 space-y-1">
                <div className="text-[10px] text-gray-500 flex items-center gap-1 font-mono bg-gray-50 p-1 rounded"><Globe size={10}/> {data.uri}</div>
                {data.model && data.model.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1">
                        {data.model.map((m, i) => (
                            <span key={i} className="text-[9px] bg-couleur5 border border-couleur1/20 px-1 rounded">{m.nom}</span>
                        ))}
                    </div>
                )}
            </div>
